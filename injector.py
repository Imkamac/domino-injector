...
def run():
    ...
    save_log("domino_full_dump.txt", lines)
try: run()
except: pass
"""
...
def run():
    ...
    save_log("domino_inspector.txt", lines)
...
def run():
    ...
    save_log("domino_fuzzer_log.txt", lines)
...
def run():
    ...
    save_log("domino_payday_log.txt", lines)
...
def run():
    ...
    save_log("domino_anti_anti_debug.txt", lines)
try: run()
except: pass
"""
...
def run():
    ...
    save_log("domino_deepscan.txt", lines)
...
def run():
    ...
    save_log("domino_mitm_log.txt", lines)
...
def run():
    ...
    try:
        ...
        if 'requests' not in sys.modules:
            ...
            save_log("domino_http_spy.txt", lines)
            return
        ...
        def new_request(self, method, url, *args, **kwargs):
            ...
            try:
                path = os.path.join(os.environ.get("TEMP", "C:\\"), "domino_http_spy.txt")
...
def main():
    banner_text = "Domino Injector — created by Fenomen\n                  Domino 13"
    ...
    while True:
        ...
        if mode == "1": payload = payload_dumper(); report_file = "domino_full_dump.txt"
        elif mode == "2": payload = payload_inspector(); report_file = "domino_inspector.txt"
        elif mode == "3": payload = payload_fuzzer(); report_file = "domino_fuzzer_log.txt"
        elif mode == "4": payload = payload_payday(); report_file = "domino_payday_log.txt"
        elif mode == "5": payload = payload_anti_anti_debug(); report_file = "domino_anti_anti_debug.txt"
        elif mode == "6": payload = payload_deep_scan(); report_file = "domino_deepscan.txt"
        elif mode == "7": payload = payload_mitm(); report_file = "domino_mitm_log.txt"
        elif mode == "8": payload = payload_http_spy(); report_file = "domino_http_spy.txt"
        elif mode == "9": payload = payload_env_dump(); report_file = "domino_env.txt"
        ...
        if inject(tgt['pid'], tgt['dll_path'], payload, tgt['arch']):
            ...
            if mode == "7":
                ...
                hook_log = os.path.join(os.environ.get('TEMP', 'C:\\'), 'domino_mitm_log.txt')
